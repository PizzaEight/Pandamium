...
execute as @e[type=#pandamium:auto_defusable,tag=!defused,tag=!pandamium.defused] at @s run function pandamium:impl/main_loop/defuse_entity/main
execute at @a[gamemode=creative,limit=1] as @e[type=#arrows,distance=..10] if items entity @s contents *[intangible_projectile] run data merge entity @s {Owner:[I;0,0,0,0],item:{components:{"!minecraft:intangible_projectile":{}}}}
# disguise (fun)
...
execute in minecraft:overworld as @e[type=item_display,tag=pandamium.ticking] at @s run function pandamium:impl/custom_entities/every_tick
