...
$execute if predicate {type:"minecraft:location_check",predicate:{position:{y:{max:$(destination)}}}} as @e[x=0,tag=elevator.id.$(id),tag=elevator.move] positioned as @s run tp @s ~ ~$(blocks_per_tick) ~
$execute if predicate {type:"minecraft:location_check",predicate:{position:{y:{max:$(destination)}}}} positioned ~ ~$(blocks_per_tick) ~ if predicate {type:"minecraft:location_check",predicate:{position:{y:{min:$(destination)}}}} as @e[x=0,tag=elevator.id.$(id),tag=elevator.move] positioned as @s run tp @s ~ $(destination) ~
# if above destination
$execute if predicate {type:"minecraft:location_check",predicate:{position:{y:{min:$(destination)}}}} as @e[x=0,tag=elevator.id.$(id),tag=elevator.move] positioned as @s run tp @s ~ ~-$(blocks_per_tick) ~
$execute if predicate {type:"minecraft:location_check",predicate:{position:{y:{min:$(destination)}}}} positioned ~ ~-$(blocks_per_tick) ~ if predicate {type:"minecraft:location_check",predicate:{position:{y:{max:$(destination)}}}} as @e[x=0,tag=elevator.id.$(id),tag=elevator.move] positioned as @s run tp @s ~ $(destination) ~
