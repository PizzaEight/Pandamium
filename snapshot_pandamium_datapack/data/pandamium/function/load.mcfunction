...
execute unless score <dev_environment> global matches 1 if score <reload_data_pack> global matches 1 run tellraw @a[predicate=pandamium:player/min_staff_perms/helper] [{color:"dark_gray",text:"[Staff Info]"},{color:"gray",text:" Data pack was updated!"}]
...
scoreboard players set <db.players.latest_data_version> global 9
...
# Rebuild the dynamic-trigger runtime + online-player cache from scratch on every load, then force
# every online player to re-run on_join on the next tick so their personal triggers are re-registered
# (home.<name>, tpa.<player>, mail.<name>, kick/ban/jail/player_info.<name>, etc.). This repopulates
# the per-tick enable commands (macros.post.lines) that keep those triggers enabled — without this,
# the triggers stay disabled even though the scoreboards/aliases look correct.
execute in minecraft:overworld if data storage pandamium.dynamic_triggers:data aliases run data remove storage pandamium.dynamic_triggers:data aliases
execute in minecraft:overworld if data storage pandamium:cache online_players run data remove storage pandamium:cache online_players
# Reset the runtime macro line tables/bounds unconditionally every load so no stale per-tick enable
# commands (or stale entries) survive a reload, and every online player's triggers get re-registered.
execute in minecraft:overworld run function pandamium:impl/dynamic_triggers/set_default_macro_arguments
execute in minecraft:overworld run function pandamium:impl/database/cache/set_default_macro_arguments
execute in minecraft:overworld run data modify storage pandamium.dynamic_triggers:data macros.main.entries set value []
execute in minecraft:overworld run data modify storage pandamium.dynamic_triggers:data macros.post.entries set value []
...
team modify wither_skull friendlyFire false
